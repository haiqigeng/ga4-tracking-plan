...
def _extract_data_layer(code: str, event_name: str, *, context: bool) -> dict[str, Any]:
    ...
    for index in range(len(values) - 1, -1, -1):
        ...
        if value.get("event") == event_name or (context and "event" not in value):
            ...
            break
    push = values[selected_index] if selected_index is not None else (
        {} if context else {"event": event_name}
    )
    clear = [
        str(key)
...
def _projection_differences(
    ...
    *,
    ...
    expected_sheets = {
        ...
        for item in expected.get("sheets", [])
        if isinstance(item, dict)
    }
    observed_sheets = {
        str(item.get("title")): item
        for item in observed.get("sheets", [])
        if isinstance(item, dict)
    }
    for title in sorted(set(expected_sheets) | set(observed_sheets)):
        ...
        before_cells = {
            str(item[0]): item[1:]
            for item in before.get("cells", [])
            if isinstance(item, list) and item
        }
        after_cells = {
            str(item[0]): item[1:]
            for item in after.get("cells", [])
            if isinstance(item, list) and item
        }
...
def _projection_changes(
    ...
    for title in sorted(set(expected_sheets) | set(observed_sheets)):
        ...
        for coordinate in sorted(set(before_cells) | set(after_cells)):
            if before_cells.get(coordinate) != after_cells.get(coordinate):
                changes.append(
                    {"kind": "cell", "sheet": title, "coordinate": coordinate}
                )
    return changes
...
def _catalog_parameter(
    ...
    for parameter in record.get("parameters", []):
        if (
            ...
            and str(parameter.get("name")) == name
            and str(parameter.get("scope", "event")) == scope
        ):
            return parameter
    return None
...
def _wording_origin(visible: Any, official: Any) -> str:
    return (
        "exact"
        if " ".join(str(visible or "").split()).casefold()
        == " ".join(str(official or "").split()).casefold()
...
def _infer_path(
    ...
    matches = sorted(
        path
        for path in flatten_push_paths(push)
        if path.rsplit(".", 1)[-1].replace("[]", "") == name
    )
    if matches:
        if scope == "item":
...
def _recover_visible_model(workbook, source: Path) -> dict[str, Any]:
    ...
    for sheet in sheets:
        ...
        if catalog_record and classification != "context":
            classification = (
                "official_ecommerce"
                if str(catalog_record.get("group", "")).casefold()
                == "online sales"
                else "official"
            )
        ...
        source_column = headers.get(
            _normalized(LABELS[language]["source_path"])
        )
        ...
        while row <= sheet.max_row:
            ...
            source_lines = (
                _cell_value(sheet, row, source_column).splitlines()
                if source_column
                else []
            )
            path = (
                ...
                else _infer_path(name, scope, push, classification)
            )
            ...
            parameter_classification = (
                "implementation"
                if name == "user_id"
                or (classification == "context" and scope == "implementation")
                else ("official" if official_parameter else "custom")
            )
            parameter_type = _canonical_type(
                _cell_value(sheet, row, type_column)
                or (official_parameter or {}).get("type")
                or "string"
            )
            ...
            parameter: dict[str, Any] = {
                ...
                or f"Identifies the recovered {name} value used by {event_name}.",
                "value_rule": value_rule
                or (
                    f"Retain the normalized value represented by {name} "
            ...
            condition = (
                _cell_value(sheet, row, condition_column)
                if condition_column
                else ""
            )
            if requirement == "conditional":
                parameter["condition"] = condition or (
                    f"Send {name} only when its source value is available for "
                    f"{event_name}; review this recovered condition before delivery."
                )
            ...
            if parameter_classification == "custom":
                parameter["custom_decision"] = {
                    "business_need": (
                        f"Preserve the distinct {name} analysis dimension "
                        f"represented by the imported {event_name} specification."
                    ),
                    "official_candidate": (
                        f"The current official {event_name} parameter table was "
                        f"checked for a {name} equivalent."
                        if catalog_record
                        else (
                            "No event-specific official parameter table is "
                            "available for this custom event."
                        )
                    ),
                    "why_not_fit": (
                        f"No official {scope}-scope parameter named {name} is "
                        f"prescribed for {event_name}."
                    ),
                }
        ...
        visible_event_definition = (
            _cell_value(sheet, 6, 2)
            or f"Identifies the recovered {event_name} measurement."
        )
        event: dict[str, Any] = {
            ...
            "trigger": _cell_value(sheet, 7, 2)
            or (
                f"Push {event_name} at the implementation moment represented "
                "by the recovered event tab."
            ),
            "locations": locations,
            "parameters": parameters,
            "data_layer": {"push": push},
            "notes": (
                "Recovered from visible workbook content. Review inferred "
                "conditions, sources, destinations, and custom decisions before delivery."
            ),
        }
        ...
        if classification == "custom":
            event["custom_decision"] = {
                "business_need": (
                    f"Preserve the distinct business action represented by the "
                    f"imported {event_name} trigger."
                ),
                "official_candidate": (
                    "The current recommended-event catalog contains no event "
                    "with this imported name."
                ),
                "why_not_fit": (
                    f"The source workbook defines {event_name} as a distinct "
                    "trigger and does not identify an equivalent recommended event."
                ),
            }
        events.append(event)
...
def _standard_event_tabs_by_name(workbook, plan: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        _cell_value(sheet, 3, 2): sheet
        for sheet in _event_tabs(workbook, language)
        if _cell_value(sheet, 3, 2)
    }
...
def _reconcile_standard_visible_edits(
    ...
    if set(tabs) != set(event_by_name):
        raise ValueError(
            "Visible edits cannot be reconciled because event tabs were added, removed, "
            "or renamed. Make structural event changes in canonical JSON."
        )
    ...
    for event_name, event in event_by_name.items():
        ...
        allowed_cells.update(
            {(sheet.title, "B6"), (sheet.title, "B7"), (sheet.title, "B9")}
        )
        code_row = next(
            (
                row
                ...
                and "window.dataLayer" in str(sheet.cell(row, 1).value)
            ),
            0,
        )
        if not code_row:
            raise ValueError(
                f'Visible edits cannot be reconciled because event tab "{sheet.title}" '
                "has no recognizable JSON-only dataLayer block."
            )
        allowed_cells.add((sheet.title, f"A{code_row}"))
        rows: list[tuple[int, dict[str, Any]]] = []
        for row in range(12, code_row):
            name = _cell_value(sheet, row, 1)
            if not name or _normalized(name) == _normalized(LABELS[
                "fr" if str(plan["document"]["language"]).casefold().startswith("fr") else "en"
            ]["datalayer"]):
                continue
            scope = SCOPE_BY_LABEL.get(_normalized(_cell_value(sheet, row, 2)))
            if scope is None:
                raise ValueError(
                    f'Unknown visible scope for parameter "{name}" on tab "{sheet.title}".'
                )
    ...
    unsupported = [
        ...
        if change.get("kind") != "cell"
        or (str(change.get("sheet")), str(change.get("coordinate"))) not in allowed_cells
    ]
    if unsupported:
        details = ", ".join(
            (
    ...
    for event_name, event in event_by_name.items():
        ...
        expected_structure = {
            "classification": classification_label(
                plan, str(event.get("classification", ""))
            ),
        ...
        parameter_by_key = {
            (str(parameter.get("name")), str(parameter.get("scope"))): parameter
            for parameter in event.get("parameters", [])
            if isinstance(parameter, dict)
        }
        if set(visible_keys) != set(parameter_by_key) or len(visible_keys) != len(parameter_by_key):
            raise ValueError(
                f'Parameters were added, removed, duplicated, or renamed on tab "{sheet.title}". '
                "Make structural parameter changes in canonical JSON."
            )
        for row, visible in rows:
            ...
            requirement = REQUIREMENT_BY_LABEL.get(
                _normalized(_cell_value(sheet, row, 4))
            )
            if requirement is None:
                raise ValueError(
                    f'Unknown requirement for parameter "{visible["name"]}" '
                    f'on tab "{sheet.title}".'
                )
            rule, allowed_values = _split_value_rule(
                _cell_value(sheet, row, 6),
                "fr"
                if str(plan["document"]["language"]).casefold().startswith("fr")
                else "en",
            )
            parameter["type"] = parameter_type
            parameter["requirement"] = requirement
            parameter["definition"] = _cell_value(sheet, row, 5)
            parameter["value_rule"] = rule
            if allowed_values:
                parameter["allowed_values"] = allowed_values
            else:
        ...
        if code != datalayer_code(event):
            ...
            if set(parsed.get("clear", [])) != set(old_data_layer.get("clear", [])):
                raise ValueError(
                    f'dataLayer clear keys changed on tab "{sheet.title}". '
                    "Change wrapper structure in canonical JSON."
                )
            if flatten_push_paths(parsed.get("push", {})) != flatten_push_paths(
                old_data_layer.get("push", {})
            ):
                raise ValueError(
                    f'dataLayer keys changed on tab "{sheet.title}". '
                    "Change parameter paths and wrapper structure in canonical JSON."
                )
    ...
    if issues:
        raise ValueError(
            "Visible edits do not produce a delivery-valid canonical model:\n"
            + render_text(issues)
        )
    return plan
...
def import_workbook(
    ...
    if embedded is not None:
        ...
        if baseline is not None and _normalized_projection(baseline) != _normalized_projection(
            ...
            details = ", ".join(
                _projection_differences(baseline, workbook_projection(workbook))
            )
            raise ValueError(
